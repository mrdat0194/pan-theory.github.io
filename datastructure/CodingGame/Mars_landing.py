...
class LandingZone():
    ...
    def getToSafeZone(self):
        if self._dir_mult != 0:
            ...
            if self._shutle.y - self._hightest_dot <50:
                l_a = DANGER_LANDING_ANGLE/1.1 -4
                # if self._shutle.y - self._hightest_dot <20:
                #     l_a = DANGER_LANDING_ANGLE/1.1 +29
                # if l_a == -4:
                #     l_a = DANGER_LANDING_ANGLE/1.1 +50
            ...
            if ((-self._dir_mult * self._shutle.h_speed) < l_s):
                ...
                # if self._shutle.angle == -30 :
                #     self._shutle.angle = -40
            elif (-self._dir_mult * self._shutle.h_speed > l_s + 5):
                self._shutle.angle = -int(self._dir_mult * l_a)
            else:
                self._shutle.angle = -30
        else:
    ...
    def gorisontalCorrect(self):
        if abs(self._shutle.h_speed) > MAX_HSPEED_LANDING  or (self._shutle.h_speed>0 and self._landing_end -self._shutle.x<500) or (self._shutle.h_speed<0 and self._landing_start -self._shutle.x<-500):            
            self._shutle.angle = self._shutle.h_speed * 4 +100
            if (self._shutle.h_speed>0 and self._landing_end -self._shutle.x<700) or (self._shutle.h_speed<0 and self._landing_start -self._shutle.x<-700):
                self._shutle.angle = self._shutle.h_speed * 4  
            # if self._shutle.angle == 0:
            #    self._shutle.angle = self._shutle.h_speed * 4 +100 
