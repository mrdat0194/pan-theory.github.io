...
def GOE(N):
    ...
    by creating an NxN matrix of Gaussian random variables
      using the random array function scipy.random.standard_normal([shape])
      with [shape] = (N,N).
    and adding it to its transpose (applying scipy.transpose to the matrix)
    #
    Typing GOE(4) a few times, check it's symmetric and random
    """
    m = scipy.random.standard_normal((N, N))
    m = m + scipy.transpose(m)
    return m
...
def CenterEigenvalueDifferences(ensemble):
    ...
    for each m in ensemble
      finds the eigenvalues
        scipy.linalg.eigvals(m) gives the eigenvalues of m
      sorts them
        scipy.sort(e) sorts a list from smallest to largest
    ...
    Converts the list of differences diff into an array.
      ...
      Arrays are scipy objects that look the same as lists, but
      can't be appended and can be multiplied.
      Use d = scipy.array(d) to convert from a list to an array.
    Takes the real part of the difference array (rounding errors)
      For a scipy d, d.real is the real part
    ...
    diffs = []
    for mat in ensemble:
        eigenvalues = scipy.sort(scipy.linalg.eigvals(mat))
        # eigenvalues = scipy.sort(np.linalg.eigvalsh(mat))
...
def CenterDiffHistogram(ensemble, bins=30, showPlot=True):
    ...
    Finds the averages diffAve of the differences.
      scipy.sum(d) gives the total; len(d) gives the num in the ensemble
...
def Wigner(s):
    ...
    Python provides a number of complications:
      ...
      pi and exp are not Python functions, but scipy functions
    ...
    """
    return (scipy.pi * s / 2.0) * scipy.exp(-scipy.pi * s ** 2 / 4.)
...
def CompareEnsembleWigner(ensemble, bins=30):
    ...
    with showPlot=False.
    Then creates an array of s values using scipy.arange from 0.0 to 4.0
    with a step of 0.01.
    If the function Wigner is defined using only multiplication, powers,
    and simple scipy functions like exponentials, Wigner(sArray) should
    ...
    sArray = scipy.arange(0.0, 4.0, 0.01)
...
def PM1(N):
    ...
    I generated an asymmetric array scipy.random.randint, and then set the
    bottom left equal to the top right in a double loop.
      #
      scipy.random.randint(min, max+1, (N,N)) will generate random integers
    ...
    and +-1.
    """
    m = scipy.random.randint(0, 2, (N, N)) * 2.0 - 1.0
    for i in range(N):
        for j in range(i):
            m[j, i] = m[i, j]
    return m
